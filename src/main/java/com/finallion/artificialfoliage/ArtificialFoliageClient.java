...
import com.finallion.artificialfoliage.utils.ARFOModelResourceProvider;
...
public class ArtificialFoliageClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        BlockRenderLayerMap.INSTANCE.putBlocks(ColorUtils.LEAVE_BLOCK_LAYER,
                ARFOBlocks.COLORABLE_GRASS_BLOCK,
                ARFOBlocks.COLORABLE_OAK_LEAVES,
                ARFOBlocks.COLORABLE_DARK_OAK_LEAVES,
                ARFOBlocks.COLORABLE_JUNGLE_LEAVES,
                ARFOBlocks.COLORABLE_ACACIA_LEAVES,
        ...
        // colorable blocks ability to change color
        // colorable blocks particle color based on colorable block color
        ModelLoadingRegistry.INSTANCE.registerVariantProvider((resourceManager -> new ARFOModelResourceProvider()));
        /*
        ColorableBlockBaseModel grassBlockBakedModel = new ColorableGrassBlockModel();
        ColorableBlockBaseModel oakLeavesBakedModel = new ColorableOakLeavesBlockModel();
        ColorableBlockBaseModel darkOakLeavesBakedModel = new ColorableDarkOakLeavesBlockModel();
        ColorableBlockBaseModel jungleLeavesBakedModel = new ColorableJungleLeavesBlockModel();
        ColorableBlockBaseModel acaciaLeavesBakedModel = new ColorableAcaciaLeavesBlockModel();


        ModelLoadingRegistry.INSTANCE.registerResourceProvider((res) -> (modelId, ctx) -> {
            if (modelId.getNamespace().equals("arfo") && modelId.getPath().equals("colorable_grass_block")) {
                return grassBlockBakedModel;
            } else if (modelId.getNamespace().equals("arfo") && modelId.getPath().equals("colorable_oak_leaves")) {
                return oakLeavesBakedModel;
        ...
        ColorProviderRegistry.BLOCK.register(new BlockColorProvider() {
            @Override
            public int getColor(BlockState state, @Nullable BlockRenderView world, @Nullable BlockPos pos, int tintIndex) {
                if (state.getBlock() instanceof IGiveColor && world != null) {
                    BlockEntity blockEntity = world.getBlockEntity(pos);
                    if (blockEntity instanceof ColorableBlockEntity) {
                        return ((ColorableBlockEntity) blockEntity).getColor();
                    } else {
        ...
        ClientPlayNetworking.registerGlobalReceiver(WandOfImitation.POS_BLOCK_CONSTANT, ((client, handler, buf, responseSender) -> {
            BlockPos position = buf.readBlockPos();
            BlockState state = client.world.getBlockState(position);
            int color = ColorProviderRegistry.BLOCK.get(state.getBlock()).getColor(state, client.world, position, 1);

            PacketByteBuf backPacket = PacketByteBufs.create();
            backPacket.writeInt(color);
            ClientPlayNetworking.send(WandOfImitation.COLOR_CONSTANT, backPacket);
