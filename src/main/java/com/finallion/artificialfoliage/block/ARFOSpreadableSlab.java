...
public class ARFOSpreadableSlab extends SlabBlock implements Fertilizable, IReturnColor {
    ...
    public boolean isFertilizable(BlockView world, BlockPos pos, BlockState state, boolean isClient) {
        if (state.isOf(ARFOBlocks.MYCELIUM_SLAB) || state.isOf(ARFOBlocks.GRASS_SLAB) || state.isOf(BYGBlocks.BYG_MEADOW_GRASS_SLAB)) {
            return false;
        }
    ...
    public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
        ...
        for (Block b1 : BlockMapping.slabAndBlocks.keySet()) {
            if (BlockMapping.slabAndBlocks.get(b1).getDefaultState().isOf(blockState.getBlock())) {
    ...
    public static void setToDirt(World world, BlockPos pos){
        ...
        } else if (state.isOf(BYGBlocks.BYG_MEADOW_GRASS_SLAB)) {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (canSpread(state, world, newPos)) {
                    ...
                    if (newState.isOf(Blocks.DIRT)) { // vanilla grass slab/mycelium to dirt
                        if ((state.isOf(ARFOBlocks.GRASS_SLAB) || state.isOf(BYGBlocks.BYG_MEADOW_GRASS_SLAB)) && !world.getBlockState(newPos.up()).getMaterial().isSolid()) {
                            world.setBlockState(newPos, Blocks.GRASS_BLOCK.getDefaultState(), 3);
                        } else if (state.isOf(ARFOBlocks.MYCELIUM_SLAB)) {
                        ...
                        if (state.isOf(ARFOBlocks.GRASS_SLAB) && !world.getBlockState(newPos.up()).getMaterial().isSolid()) {
                        ...
                        if (state.isOf(BYGBlocks.BYG_MEADOW_GRASS_SLAB) && !world.getBlockState(newPos.up()).getMaterial().isSolid()) {
                            if (newState.get(TYPE) == SlabType.BOTTOM) {
